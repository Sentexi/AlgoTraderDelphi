        ...
        if os.path.exists(csv_path):
            ...
            if 'actual' in df.columns and 'prediction' in df.columns:
                ...
                for i in range(1, len(df)):
                    # Define the range of ±0.5% around the last actual value
                    lower_bound = df.loc[i - 1, 'actual'] * 0.9975
                    upper_bound = df.loc[i - 1, 'actual'] * 1.0025

                    # Check if the prediction falls within the range
                    if lower_bound <= df.loc[i, 'prediction'] <= upper_bound:
                        predictions.append(0)  # Set to 0 if within ±0.5% range
...
plt.tight_layout()
plt.savefig('simulation_safe.png')
plt.show()