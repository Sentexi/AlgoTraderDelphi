...
folder_path <- "G:/Meine Ablage/LM DayTrader/stock_data_daily/"

# List all .csv files in the folder
csv_files <- list.files(path = folder_path, pattern = "\\.csv$", full.names = TRUE)

# Initialize a results data frame outside the loop
results_df <- data.frame(
  ticker = character(),
  last_closing_price = numeric(),
  forecast_mean = numeric(),
  forecast_lower = numeric(),
  forecast_upper = numeric(),
  expected_return_mean = numeric(),
  expected_return_lower = numeric(),
  expected_return_upper = numeric(),
  stringsAsFactors = FALSE
)

# Loop over each file in csv_files
for (file_path in csv_files) {
  
  # Extract the stock ticker from the file name (assuming file names are like 'AAPL.csv')
...
price_data <- price_data %>% fill(everything(), .direction = "down") |> head(509)
...
target_col <- paste0("close_", ticker)         # Use closing price of Exxon Mobil as target
...
add_custom_features <- function(df, stock) {
  # Dynamically construct the column names based on the stock ticker
  open_col <- paste0("open_", stock)
  close_col <- paste0("close_", stock)
  high_col <- paste0("high_", stock)
  low_col <- paste0("low_", stock)
  
  if (all(c(open_col, close_col, high_col, low_col) %in% names(df))) {
    df <- df %>%
      mutate(
        !!paste0(stock, "_up_down") := ifelse(!!sym(close_col) > !!sym(open_col), 1, 0),
        !!paste0(stock, "_daily_range") := !!sym(high_col) - !!sym(low_col),
        !!paste0(stock, "_relative_range") := (!!sym(high_col) - !!sym(low_col)) / !!sym(close_col)
      )
  } else {
    warning(paste("Some columns are missing for stock:", stock))
  }
...
for (stock in stocks) {
  ...
  price_data <- add_custom_features(price_data, stock)
...
columns_to_keep <- grep("(_lag|datetime|target|_SMA|_BB|_MACD|_ADX|_Stoch|_CCI|_ATR|_WilliamsR|_up_down|_daily_range|_relative_range)", names(price_data), value = TRUE)
...
actualpredicted <- ggplot(test_data, aes(x = as.POSIXct(datetime, format="%Y-%m-%d %H:%M:%S"))) +
...
ggsave(plot = actualpredicted, filename = paste0(folder_path, ticker, "_model_fit.png"), width = 10, height = 8, dpi = 600)
...
ggsave(plot = forecast_with_interval, filename = paste0(folder_path, ticker, "_forecast.png"), width = 10, height = 8, dpi = 600)
...
xgb.save(final_model, paste0(ticker, ".model"))


# Append the results for the current stock to the results data frame
results_df <- rbind(
  results_df,
  data.frame(
    ticker = ticker,
    last_closing_price = last_closing_price,
    forecast_mean = first_forecast_mean,
    forecast_lower = first_forecast_lower,
    forecast_upper = first_forecast_upper,
...
write.csv(results_df, file = paste0(folder_path, "forecast_results_summary.csv"), row.names = FALSE)
